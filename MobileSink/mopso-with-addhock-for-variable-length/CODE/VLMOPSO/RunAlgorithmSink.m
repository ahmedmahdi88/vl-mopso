function [cccc,pop,env,paretoFront,paretoSet ,NC,classes]=RunAlgorithm(nSensors,scenario,objfun,popSize,nobj,RepSize,lowerBound_pos,heigherBound_pos,lowerBound_dim,higherBound_dim,nGrid,alpha,numberOfIter,w)
...
% [ind1,ind2]= Reconciliation(tmpPbest,pop(i).pos);
% [ind3,ind4]= Reconciliation(tmpRep,pop(i).pos);
%  pop(i).vel.x(ind2)= w*pop(i).vel.x(ind2)+ rand*(tmpPbest.x(ind1)-pop(i).pos.x(ind2)) ;
%  pop(i).vel.y(ind2)= w*pop(i).vel.y(ind2)+ rand*(tmpPbest.y(ind1)-pop(i).pos.y(ind2)) ;
%  pop(i).vel.x(ind4)=w*pop(i).vel.x(ind4)+ rand*(tmpRep.x(ind3)-pop(i).pos.x(ind4)) ;
%  pop(i).vel.y(ind4)=w*pop(i).vel.y(ind4)+ rand*(tmpRep.y(ind3)-pop(i).pos.y(ind4)) ;
 miin=min([pop(i).Dim,length(tmpPbest.x(tmpPbest.x~=-1)),length(tmpRep.x(tmpRep.x~=-1))]);
    pop(i).vel.x(1:miin)= w*pop(i).vel.x(1:miin)+ rand*(tmpPbest.x(1:miin)-pop(i).pos.x(1:miin)) +rand*(tmpRep.x(1:miin)-pop(i).pos.x(1:miin));
    pop(i).vel.y(1:miin)= w*pop(i).vel.y(1:miin)+ rand*(tmpPbest.y(1:miin)-pop(i).pos.y(1:miin)) +rand*(tmpRep.y(1:miin)-pop(i).pos.y(1:miin));

        pop(i).vel.x(pop(i).Dim+1: end)=0;
        pop(i).vel.y(pop(i).Dim+1: end)=0;
        tmp=pop(i).pos;
        tmp.x(1:miin)=  pop(i).pos.x(1:miin) + pop(i).vel.x(1:miin);
        tmp.y(1:miin)=  pop(i).pos.y(1:miin) + pop(i).vel.y(1:miin);
        %         tmp=pop(i).pos;

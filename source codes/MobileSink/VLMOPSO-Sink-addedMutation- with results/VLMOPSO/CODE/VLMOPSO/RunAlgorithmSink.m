...
% w: inertia weight
function [countMut,countMutRef,cccc,countIsFixed,pop,env,enhancement_timeout,paretoFront,paretoSet ,NC, classes]=RunAlgorithm(RepSize,nGrid,alpha,nSensors,scenario,objfun,popSize,nobj,lowerBound_pos,heigherBound_pos,lowerBound_dim,higherBound_dim,numberOfIter,w,mutationProbability,mutationRatio)
...
%             tmpAMC(i).vel.y=zeros(size(tmpAMC(i).vel.x));
            ...
            [tmp,countMut,countMutRef] = mutation(countMut,countMutRef,tmp,mutationProbability,mutationRatio,env,covThreshold,sensorPos,lowerBound_pos,heigherBound_pos);
            DistThreShold=3;

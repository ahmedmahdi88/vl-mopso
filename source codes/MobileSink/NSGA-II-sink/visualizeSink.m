function visualizeSink(env,pos,idx,paretoFront)
W=20;
H=20;
sensorsRadius=1;
rendiRadius=2;
...
for is=1:length(env.sensors)
        plot(env.sensors(is).x,env.sensors(is).y,'bp');xlim([0 W]);ylim([0 H]);
        hold on;
...
for ir=1:length(env.rndVoz)
    if env.rndVoz(ir).x>=0 && env.rndVoz(ir).y>=0
        plot(env.rndVoz(ir).x,env.rndVoz(ir).y,'ko');xlim([0 W]);ylim([0 H]);
        hold on;
...
title(['Time: ' num2str(paretoFront.solutionsObjectiveValues(idx,1)) ' | Energy: ' num2str(paretoFront.solutionsObjectiveValues(idx,2))]);
hold off;
end

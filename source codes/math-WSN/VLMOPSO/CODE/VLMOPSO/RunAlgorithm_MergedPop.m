function [pop,t,population,maxEnhancementTimeOut,paretoFront,paretoSet ,NC, classes]=...
...
% Run the non-dominating sorting algorithm
...
maxEnhancementTimeOut=round(numberOfIter/10)-1;
...
%     p_mutation = p_mutation0*(-a*x^2 + 1);
    p_mutation = p_mutation0*exp(-iter/numberOfIter);
%     w =  w0*(-a*x^2 + 1);
    ...
    %*************************************************************
    %% Movement the particles based on AMC matrices
    for c=1:NC
        ...
        for i=1:length(tmpAMC)
            ...
            tmp_cost = objfun(tmp);
            newPop(newPopCounter).pos = tmp;
            newPop(newPopCounter).cost = tmp_cost;
            newPop(newPopCounter).vel = tmpAMC(i).vel;
            newPop(newPopCounter).IsDominated = tmpAMC(i).IsDominated;
            ...
            newPop(newPopCounter).cont = tmpAMC(i).cont;
            ...
            newPop(newPopCounter).pbest = tmpAMC(i).pbest;
             newPop(newPopCounter).GridIndex = [];
            newPop(newPopCounter).GridSubIndex = [];
            newPop(newPopCounter).class = classes(c);
            newPop(newPopCounter).DominationSet=[];
            newPop(newPopCounter).DominatedCount=0;
            newPop(newPopCounter).Rank=0;
            newPop(newPopCounter).CrowdingDistance=0;
            newPopCounter = newPopCounter + 1;
        end
        eval([str '=tmpAMC' ]); %update AMC matrix for the current class
    end
   %% Merging populations
   mergedPop = [pop newPop];
   mergedPop = DetermineDomination(mergedPop);
   % Run the non-dominating sorting algorithm
        [mergedPop,F] = NonDominatedSorting(mergedPop);
        % calculate crowding distance
        mergedPop = CalcCrowdingDistance(mergedPop,F);
        % Sort Population
        [mergedPop, F] = SortPopulation(mergedPop);
        pop = mergedPop(1:popSize);
        classes = [pop.class];
...
for i=1:NC
    ...
    for j=1:NC
        ...
        for i=1:length(tmp1)
            if tmp1(i).cont>1 && length(tmp1)>0 % if the particle isn't improved within...
                %10 iterations
                tmp1(i).cont=0;
                newClassTemp=tmp1(i);

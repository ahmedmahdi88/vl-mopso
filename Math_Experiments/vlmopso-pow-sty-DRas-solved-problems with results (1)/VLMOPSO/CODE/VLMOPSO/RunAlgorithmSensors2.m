...
% w: inertia weight
function [population,enhancement_timeout,paretoFront,paretoSet ,NC, classes,v]=RunAlgorithm(scenario,objfun,popSize,nobj,lowerBound_pos,heigherBound_pos,lowerBound_dim,higherBound_dim,numberOfIter,w,minNumOfParticles)
 ...
 for j=1:NC
     if NC==1
      ...
      index=[];
       ...
       tmp1= eval(str1);
       for i=1:length(tmp1)
           if tmp1(i).cont>enhancement_timeout && length(tmp1)>minNumOfParticles % if the particle isn't improved within... 
                                  %enhancement_timeout
               tmp1(i).cont=0;
               newClassTemp=tmp1(i);

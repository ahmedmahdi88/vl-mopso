...
hold on; 
plot(C_MO_VL,'linewidth',2);legend({'C(VL-MOPSOMOPSO)','C(MOPSO,VL-MOPSO)'});
...
suptitle(['ttest ']);
p=pwd;
saveas(f1,[p '\rosenLevyFigs\SetCoverage(mopsomut-vlmopso)-rl-allScenarios.png']);
saveas(f3,[p '\rosenLevyFigs\SetCoverage(mopso-vlmopso)-rl-allScenarios.png']);
saveas(f2,[p '\rosenLevyFigs\SetCoverage(nsga2-vlmopso)-rl-allScenarios.png']);
saveas(f4,[p '\rosenLevyFigs\SetCoverage-ttest-rl-allScenarios.png']);
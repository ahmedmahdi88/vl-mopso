...
[~,pvmm]=ttest(C_VL_MOmut,C_MOmut_VL);
[~,pvn]=ttest(C_VL_N2,C_N2_VL);
somedata=[pvm pvmm pvn];
...
suptitle(['ttest ']);
p=pwd;
saveas(f1,[p '\images\SetCoverage(mopso-vlmopso)-lsp-allScenarios.png']);
saveas(f2,[p '\images\SetCoverage(nsga2-vlmopso)-lsp-allScenarios.png']);
saveas(f3,[p '\images\SetCoverage(mopsomut-vlmopso)-lsp-allScenarios.png']);
saveas(f4,[p '\images\SetCoverage-ttest-lsp-allScenarios.png']);
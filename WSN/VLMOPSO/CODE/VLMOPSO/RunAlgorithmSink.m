...
% w: inertia weight
function [pop,env,population,enhancement_timeout,paretoFront,paretoSet ,NC, classes,v]=RunAlgorithm(nSensors,scenario,objfun,popSize,nobj,lowerBound_pos,heigherBound_pos,lowerBound_dim,higherBound_dim,numberOfIter,wmutationProbability,mutationRatio)
...
%      plot(pdfpf);
    for i=1:length(tmpAMC)
        ...
        tmpAMC(i).vel.x= w*tmpAMC(i).vel.x+ rand*(tmpAMC(i).pbest.x-tmpAMC(i).pos.x) +rand*(tmpAMC(h).pos.x-tmpAMC(i).pos.x);
        tmpAMC(i).vel.y= w*tmpAMC(i).vel.y+ rand*(tmpAMC(i).pbest.y-tmpAMC(i).pos.y) +rand*(tmpAMC(h).pos.y-tmpAMC(i).pos.y);
tmp=[];
        tmp.x=  (tmpAMC(i).pos.x + tmpAMC(i).vel.x); 
         tmp.y=  (tmpAMC(i).pos.y + tmpAMC(i).vel.y);
         tmp=mutation(p,mutationProbability,mutationRatio);
         [tmp]=checkBoundsSink(tmp,env);
       ...
       for i=1:length(tmp1)
           if tmp1(i).cont>enhancement_timeout && length(tmp1)>5 % if the particle isn't improved within... 
              ...
              % zero padding when move to higher class
               ...
               newClassTemp.pos=checkBoundsSink(newClassTemp.pos,env);
               cov=computeCov(newClassTemp.pos,env);
%                while(cov<0.8)
%                newClassTemp.pos.x=round(unifrnd(0,19.9,1,length(newClassTemp.pos.x)),1);
%                newClassTemp.pos.y=round(unifrnd(0,19.9,1,length(newClassTemp.pos.y)),1);
%                newClassTemp.pos=checkBoundsSink(newClassTemp.pos,env);
%                cov=computeCov(newClassTemp.pos,env);
%                end
                   while(cov<0.8)
                   newX=zeros(size(newClassTemp.pos.x));
         newY=newX;
         for nxy=1:length(newX)
     rndIndx=round(unifrnd(1,length(xSensors)));
     newX(nxy)=round(unifrnd(xSensors(rndIndx)+0.1,xSensors(rndIndx)+3),1);
     newY(nxy)=round(unifrnd(ySensors(rndIndx)+0.1,ySensors(rndIndx)+3),1);
                ...
                newClassTemp.pos.x=newX;%round(unifrnd(0,19.9,1,length(tmp.x)),1);
                newClassTemp.pos.y=newY;%round(unifrnd(0,19.9,1,length(tmp.y)),1);
                [newClassTemp.pos]=checkBoundsSink(newClassTemp.pos,env);
                newClassTemp.pos.x(newClassTemp.pos.x<lowerBound_pos)=lowerBound_pos;
                newClassTemp.pos.y(newClassTemp.pos.y<lowerBound_pos)=lowerBound_pos;
                newClassTemp.pos.x(newClassTemp.pos.x>higherBound_pos)=higherBound_pos;
                newClassTemp.pos.y(newClassTemp.pos.y>higherBound_pos)=higherBound_pos;

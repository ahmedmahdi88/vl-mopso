...
%% full coverage vlmopso visualization
...
%% random mopso solution visualization
randomIndexWM= round(unifrnd(1,size(paretoSetwm,1)));
randPosWM=paretoSet(randomIndexWM,paretoSet(randomIndexWM,:)~=0);
%% full coverage mopso visualization
indexWM=find(paretoFrontwm(:,1)==0,1);
if isempty(indexWM)
    disp('can not find mopso coverage objective=1');
else
fullCovPosWM=paretoSetwm(indexWM,paretoSet(indexWM,:)~=0);
end
%% random NSGA-II solution visualization
randomIndexN= round(unifrnd(1,size(pns,1)));
randPosN=pns(randomIndexN).pos(1:pns(randomIndexN).dim);
%% full coverage NSGA-II visualization
...
title(['full coverage | cov= ' num2str(1-pno(indexN,1)) ' | sensors= ' num2str(length(indexN))]);
suptitle('NSGA-II');
...
title(['random coverage | cov= ' num2str(1-pno(randomIndexN,1)) ' | sensors= ' num2str(length(randPosN))]);
suptitle('NSGA-II');
...
title('NSGA-II');
%% saving
saveas(f1,[p '/../../../images/fullCov-SC-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f2,[p '/../../../images/randCov-SC-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f3,[p '/../../../images/fullCov-m-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f4,[p '/../../../images/randCov-m-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f5,[p '/../../../images/fullCov-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f6,[p '/../../../images/randCov-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f7,[p '/../../../images/fullCov-nsga2-scenario-' num2str(Scenario) '.png']);
saveas(f8,[p '/../../../images/randCov-nsga2-scenario-' num2str(Scenario) '.png']);
saveas(f9,[p '/../../../images/paretoWithLength-SC-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f10,[p '/../../../images/paretoWithLength-m-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f11,[p '/../../../images/paretoWithLength-MOPSO-scenario-' num2str(Scenario) '.png']);
saveas(f12,[p '/../../../images/paretoWithLength-nsga2-scenario-' num2str(Scenario) '.png']);
%%
clc;
disp('save is done');
%% the end